...
lib = CDLL("/users/duanyou/backup_c6/v3tiny_experiments/1_v3tiny_diou/diou_darknet/libdarknet.so", RTLD_GLOBAL)
...
predict_image_cls = lib.network_predict_image
predict_image_cls.argtypes = [c_void_p, IMAGE]
predict_image_cls.restype = POINTER(c_float)
...
if __name__ == "__main__":
    ...
    #print r[:10]
    # for i in range(10):
        net = load_net(b"/users/maqiao/mq/Data_checked/multiClass/backup_yolov3-spp/multiClass_yolov3-spp_test.cfg", 
    ...
    # import cv2
    # img = cv2.imread("/mnt/diskc/xiaofan/darknet_2019/data/car_test.jpg")
    # for detect in r:
    #     cv2.rectangle(img, (int(detect[2][0])-32, int(detect[2][1])-32), 
    #                        (int(detect[2][0])+int(detect[2][2]), int(detect[2][1])+int(detect[2][3])), 
    #                        (0, 0, 255), 3)
