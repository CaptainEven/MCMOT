    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
            ...
            if opt.id_weight > 0:  # do tracking(detection and re-id)
                save_model(os.path.join(opt.save_dir, 'mcmot_last_track_' + opt.arch + '_visdrone.pth'),
                           epoch, model, optimizer)
            else:  # only do detection
                # save_model(os.path.join(opt.save_dir, 'mcmot_last_det_' + opt.arch + '_visdrone.pth'),
                #        epoch, model, optimizer)
                save_model(os.path.join(opt.save_dir, 'mcmot_last_det_' + opt.arch + '_visdrone.pth'),
                           epoch, model, optimizer)
        logger.write('\n')
